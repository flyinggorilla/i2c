...
uint8_t StUsb4500::Init(uint8_t deviceAddress, i2c_port_num_t i2cPort)
{
  readSectors = 0;
  _deviceAddress = deviceAddress; // If provided, store the I2C address from user
  mI2cPort = i2cPort;             // Grab which port the user wants us to use
...
void StUsb4500::read(void)
{
  ...
  if (currentValue == 0)
    ...
    setCurrent(1, currentValue * 0.25 + 0.25);
  else
    setCurrent(1, currentValue * 0.50 - 2.50);
  ...
  if (currentValue == 0)
    ...
    setCurrent(2, currentValue * 0.25 + 0.25);
  else
    setCurrent(2, currentValue * 0.50 - 2.50);
  ...
  if (currentValue == 0)
    ...
    setCurrent(3, currentValue * 0.25 + 0.25);
  else
    setCurrent(3, currentValue * 0.50 - 2.50);
}
...
void StUsb4500::write(uint8_t defaultVals)
{
  if (defaultVals == 0)
  {
    ...
    for (byte i = 0; i < 3; i++)
    {
      ...
      if (current > 5.0)
        current = 5.0; // Constrain current value to 5A max
...
uint8_t StUsb4500::CUST_EnterWriteMode(unsigned char ErasedSector)
{
  ...
  if (I2C_Write_USB_PD(FTP_CUST_PASSWORD_REG, Buffer, 1) != 0)
    return -1;

  Buffer[0] = 0; /* this register must be NULL for Partial Erase feature */
  if (I2C_Write_USB_PD(RW_BUFFER, Buffer, 1) != 0)
    return -1;

  {
    ...
    if (I2C_Write_USB_PD(FTP_CTRL_0, Buffer, 1) != 0)
      return -1;
  ...
  if (I2C_Write_USB_PD(FTP_CTRL_1, Buffer, 1) != 0)
    return -1; /* Set Write SER Opcode */

  Buffer[0] = FTP_CUST_PWR | FTP_CUST_RST_N | FTP_CUST_REQ;
  if (I2C_Write_USB_PD(FTP_CTRL_0, Buffer, 1) != 0)
    return -1; /* Load Write SER Opcode */

  do
  {
    delay(500);
    if (I2C_Read_USB_PD(FTP_CTRL_0, Buffer, 1) != 0)
      return -1; /* Wait for execution */
  } while (Buffer[0] & FTP_CUST_REQ);

  Buffer[0] = SOFT_PROG_SECTOR & FTP_CUST_OPCODE;
  if (I2C_Write_USB_PD(FTP_CTRL_1, Buffer, 1) != 0)
    return -1; /* Set Soft Prog Opcode */

  Buffer[0] = FTP_CUST_PWR | FTP_CUST_RST_N | FTP_CUST_REQ;
  if (I2C_Write_USB_PD(FTP_CTRL_0, Buffer, 1) != 0)
    return -1; /* Load Soft Prog Opcode */
  ...
  if (I2C_Write_USB_PD(FTP_CTRL_1, Buffer, 1) != 0)
    return -1; /* Set Erase Sectors Opcode */

  Buffer[0] = FTP_CUST_PWR | FTP_CUST_RST_N | FTP_CUST_REQ;
  if (I2C_Write_USB_PD(FTP_CTRL_0, Buffer, 1) != 0)
    return -1; /* Load Erase Sectors Opcode */
...
uint8_t StUsb4500::CUST_WriteSector(char SectorNum, unsigned char *SectorData)
{
  ...
  if (I2C_Write_USB_PD(RW_BUFFER, SectorData, 8) != 0)
    return -1;
